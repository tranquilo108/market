...
def login(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            session_key = request.session.session_key  # Создание сессионного ключа для возможности переброски
            # корзины неавторизованного пользователя к впоследствии авторизованному
            ...
            if user:
                ...
                if session_key:
                    Cart.objects.filter(session_key=session_key).update(user=user)  # Присваивание сессионного ключа
